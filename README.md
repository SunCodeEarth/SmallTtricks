...
Of course, ESRI people do not want you to leave ArcGIS products and choose those powerful geospatial packages in Python such as geopandas. Therefore, we must install those packages by ourselves, which is where the frustruation comes from. But the process is no different from what we would do with a freshly installed Python 3. ArcGIS Pro also use Anaconda to manage the Python environments. If you are fine with the default Python version, you can just use the preinstalled conda to install new packages. If you are not a big fan of conda, just run ```python -m pip install packagename``` to install them. As we all know, Python packages are in the hell of dependency networks. Quite often, we need to compile and install some other pacakges first. If you have specific problems with a package, just Google it. More often, using a pre-compiled packages such as those at https://www.lfd.uci.edu/~gohlke/pythonlibs would solve the problem. 
...
- in the end, you will see the lovely messages like ```Successfully installed geopandas-0.4.0```.  
